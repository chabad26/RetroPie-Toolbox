...
INPUT_CSV = Path("/home/retropie/Documents/save_retropie/Manuels/rapports/liste-jeux-pour-manuels-clean.csv")
MANUALS_ROOT = HOME / "RetroPie" / "manuals"
GAMELISTS_ROOT = HOME / ".emulationstation" / "gamelists"
REPORT_PATH = Path("/home/retropie/Documents/save_retropie/Manuels/rapports/rapport-manuels-archive-cute.csv")
STATE_PATH = Path("/home/retropie/Documents/save_retropie/Manuels/rapports/moisson-state.json")
...
def save_state(system: str, index: int, mode: str, total: int) -> None:
...
def handle_sigint(signum, frame):
    global STOP_REQUESTED, STOP_COUNT
    ...
    if STOP_COUNT == 1:
        ...
        print()
        print("🛑 Arrêt demandé.")
        print("🌾 Je termine le jeu en cours, je sauvegarde la position, puis je range la faux.")
        print("   Appuie une deuxième fois sur Ctrl+C pour forcer l'arrêt immédiat.")
    else:
        print()
        print("💥 Arrêt immédiat demandé.")
...
def list_systems(rows: list[dict]) -> list[str]:
    systems = sorted({row["system"] for row in rows if row.get("system")})
    return systems


def choose_system_interactive(systems: list[str]) -> tuple[list[str], bool]:
    print("🌾 Moissonneuse de manuels RetroPie")
    print()
    print("Choisis la console :")

    for number, system in enumerate(systems, start=1):
        print(f"[{number}] {system}")

    print("[A] Toutes les consoles")
    print("[0] Quitter")
    print()
    ...
    if choice in {"0", "q", "quit"}:
        raise SystemExit("Sortie demandée.")

    if choice in {"a", "all", "toutes", "tout"}:
        return systems, True

    try:
        index = int(choice) - 1
        if 0 <= index < len(systems):
            return [systems[index]], False
    except ValueError:
        pass
...
def ask_resume(state: dict, rows_count: int) -> bool:
    system = state.get("system", "?")
    index = int(state.get("index", 0))
    total = int(state.get("total", rows_count))
    updated_at = state.get("updated_at", "?")

    print("💾 Session précédente trouvée")
    print(f"   Console : {system}")
    print(f"   Position : {index}/{total}")
...
def process_system(args, system: str, rows: list[dict], start_index: int = 0) -> tuple[list[dict], dict]:
    ...
    if start_index > 0:
        print(f"🔁 Reprise à la position : {start_index + 1}/{total}")

    print()

    for zero_index, row in enumerate(rows[start_index:], start=start_index):
        index = zero_index + 1
        ...
        if is_valid_pdf(destination):
            ...
            save_state(system, index, mode, total)

            if STOP_REQUESTED:
                print()
                print(f"💾 Position sauvegardée : {system}, {index}/{total}")
                return report_rows, mappings

            continue

        if destination.exists() and not is_valid_pdf(destination):
...
def main():
    ...
    if args.choose or (not args.system and not args.all):
        selected_systems, all_mode = choose_system_interactive(systems)
    elif args.all:
    ...
    start_state_used = False
    all_report_rows = []
    all_mappings = {}

    if state and not args.restart:
        state_system = state.get("system")

        if state_system in selected_systems:
            rows_for_state = [
                row for row in all_rows
                if row["system"] == state_system
            ]

            should_resume = args.resume or ask_resume(state, len(rows_for_state))

            if should_resume:
                selected_systems = selected_systems[selected_systems.index(state_system):]
                start_state_used = True
            else:
                clear_state()
    ...
    for system in selected_systems:
        ...
        if start_state_used and state and state.get("system") == system:
            start_index = int(state.get("index", 0))

        report_rows, mappings = process_system(
            args,
            system,
            rows,
        ...
        if mappings:
            all_mappings[system] = mappings

        write_report(all_report_rows)
        print()
        print(f"📊 Rapport : {REPORT_PATH}")

        if args.download and mappings:
            update_gamelist(system, mappings)

        if STOP_REQUESTED:
            print()
            print("🌾 Moisson arrêtée proprement.")
            return
